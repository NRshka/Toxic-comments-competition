...
class DataLoader(object):
    ...
    def load_clean_words(self, path):
        ...
        with open(embedding_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                try:
                    ...
                    logging.error("Err on %s" % values[:2])
        
        logging.info('Total %s word vectors.' % len(embeddings_index))
        return embeddings_index

class DataTransformer(object):
    def __init__(self, max_num_words, max_sequence_length, char_level):
        self.data_loader = DataLoader()
        self.clean_word_dict = self.data_loader.load_clean_words(config.CLEAN_WORDS_PATH)
        self.train_df = self.data_loader.load_dataset(config.TRAIN_PATH)
        self.test_df = self.data_loader.load_dataset(config.TEST_PATH)
    ...
    def prepare_data(self):
        list_sentences_train = self.train_df["comment_text"].fillna("empty").values
        list_classes = ["toxic", "severe_toxic", "obscene", "threat", "insult", "identity_hate"]
        list_sentences_test = self.test_df["comment_text"].fillna("empty").values
